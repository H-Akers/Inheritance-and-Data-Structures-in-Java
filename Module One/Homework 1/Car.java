...
public class Car extends Vehicle {
	...
	public void refuel(int amountOfGas) {
		...
		this.gasRemained = amountOfGas + this.gasRemained >= this.maxGasAmountInTank ? this.maxGasAmountInTank : this.gasRemained + amountOfGas;
	...
	public void run(int hour) {
      ...
      int consumedGas = this.gasConsumedPerHour * hour;
      if (this.gasRemained - consumedGas <= 0) {
        System.out.println("Oops, gas out! Please add fuel!");
        this.totalGasConsumed += this.gasRemained;
        this.gasRemained = 0;
      }
      else {
        this.gasRemained -= consumedGas;
        this.totalGasConsumed += consumedGas;
      }
    }
